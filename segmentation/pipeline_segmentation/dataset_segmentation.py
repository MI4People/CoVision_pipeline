...
# Custom packages
from utils_segmentation.utils_dataset import compute_bounding_box_coordinates
...
class LFASegmentationDataset:
    ...
    def resize_image(self, image):
        """
        Resize image to the height specified in args, preserving the original width/height ratio.
        """
        ratio = image.shape[1] / image.shape[0]
        new_width = int(self.args.resize_height*ratio)
        image = cv2.resize(image, (new_width, self.args.resize_height))
        return image
